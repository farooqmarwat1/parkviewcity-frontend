...
function Dropdown({
  ...
  return (
    <div ref={ref} className="relative w-full min-w-0 sm:w-auto sm:flex-1">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
  ...
  return (
    <div className="pointer-events-auto absolute inset-x-0 bottom-4 z-20 flex justify-center px-6 sm:px-16">
      <form
        ...
        className="flex w-full max-w-[1200px] flex-col divide-y divide-white/10 rounded-2xl border border-white/15 bg-white/10 backdrop-blur-2xl sm:grid sm:grid-cols-3 sm:items-center sm:divide-x sm:divide-y-0 sm:rounded-full"
      >
        <Dropdown
        ...
        />
        <div className="flex min-w-0 items-center justify-center px-4 py-3 sm:px-6 sm:py-2">
          <button type="submit" className="flex h-[48px] w-full items-center justify-center gap-2 whitespace-nowrap rounded-full border border-white/40 px-7 font-roboto text-[10px] uppercase leading-none tracking-[0.22em] text-white/60 transition-colors duration-200 hover:border-pvc-gold hover:text-pvc-gold cursor-pointer sm:w-[70%] sm:min-w-[200px] sm:max-w-[300px]">
